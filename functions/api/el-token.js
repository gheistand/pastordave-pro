...
export async function onRequestGet(context) {
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: 'Invalid token', detail: err.message }), {
      status: 401,
      headers: { 'Content-Type': 'application/json' },
    });
  }
  ...
  if (user.tier === 'free') {
    ...
    if (conversationsToday >= 2) {
      return new Response(
        JSON.stringify({
          error: 'limit_reached',
          message: "You've used your 2 free conversations today. Upgrade to Pro for unlimited access.",
        }),
